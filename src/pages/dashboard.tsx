...
import { ClientListPage as ShowCaseList } from 'pages/clients';

function DashboardPage() {
  const { session } = useSession();

  const warehouseStat = useSWR<PaginatedInterface<WarehouseInterface>>(
    () => `/warehouses`,
    () => getWarehouses({ limit: 1, offset: 0 }),
  );

  const clientStat = useSWR<PaginatedInterface<ClientInterface>>(
    () => `/clients`,
    () => getClients({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Warehouses',
      result: warehouseStat?.data?.totalCount,
      isLoading: warehouseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Clients',
      result: clientStat?.data?.totalCount,
      isLoading: clientStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Inventories',

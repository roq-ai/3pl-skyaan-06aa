...
export function InventoryListPage(props: InventoryListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getInventories({
        relations: ['company', 'shipment.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['product_name.contains', 'product_description.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'quantity', header: 'quantity', accessorKey: 'quantity' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.received_date ? format(parseISO(record?.received_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('shipment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.shipment,
        }
      : null,
